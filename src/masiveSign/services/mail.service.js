        ...
        <div style="max-width: 600px; margin: 0 auto;">
            ...
            <div style="background: #ffffff; padding: 40px 30px; border-radius: 0 0 20px 20px; box-shadow: 0 4px 6px rgba(0, 163, 180, 0.1);">
                ...
                <div style="background: linear-gradient(135deg, rgba(0, 163, 180, 0.05) 0%, rgba(227, 231, 102, 0.05) 100%); border-left: 4px solid #00A3B4; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
                    ...
                    <p style="color: #334155; font-size: 15px; line-height: 1.8; margin: 15px 0 0 0;">
                        <strong>Número de solicitud: #{{solicitudId}}</strong> 
                    </p>
                </div>
...
// ==================== PLANTILLA CREACION SOLICITUD PARA SOLICITANTE ====================
...
// ==================== PLANTILLA FINALIZACION SOLICITUD PARA SOLICITANTE ====================
const htmlContentForCompletionRequester = `
    <!DOCTYPE html>
    <html lang="es">
    <head>
        <meta charset="UTF-8">
    ...
    <body style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); padding: 20px; margin: 0;">
        ...
        <div style="max-width: 600px; margin: 0 auto;">
            ...
            <div style="background: #ffffff; padding: 40px 30px; border-radius: 0 0 20px 20px; box-shadow: 0 4px 6px rgba(0, 163, 180, 0.1);">
                ...
                <!-- Mensaje de éxito -->
                <div style="background: linear-gradient(135deg, rgba(40, 167, 69, 0.05) 0%, rgba(227, 231, 102, 0.05) 100%); border-left: 4px solid #28a745; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
                    <p style="color: #334155; font-size: 15px; line-height: 1.8; margin: 0;">
                        <strong style="color: #28a745;">✅ ¡Todas las firmas completadas!</strong><br><br>
                        Nos complace informarte que todos los firmantes han completado exitosamente el proceso de firma de tu solicitud. Los documentos están listos para ser descargados.
                    </p>
                </div>

                <!-- Resumen de la solicitud completada -->
                <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
                    <h3 style="color: #00A3B4; font-size: 14px; font-weight: 600; text-transform: uppercase; margin-bottom: 15px; letter-spacing: 0.5px;">
                ...
                <!-- Próximos pasos -->
                <div style="margin-bottom: 30px;">
                    <h3 style="color: #28a745; font-size: 14px; font-weight: 600; text-transform: uppercase; margin-bottom: 15px; letter-spacing: 0.5px;">
                        📥 ¿Qué hacer ahora?
                    </h3>
                    <ol style="color: #334155; font-size: 14px; line-height: 1.8; padding-left: 20px;">
                        <li style="margin-bottom: 10px;"><strong>Descarga los documentos firmados</strong> desde tu panel de control</li>
                        <li style="margin-bottom: 10px;"><strong>Verifica que todos los documentos</strong> tengan las firmas correctas</li>
                        <li>Conserva los documentos en un lugar seguro para futuras referencias</li>
                    </ol>
                </div>

                <!-- Información adicional -->
                <div style="background: #e8f5e9; border-left: 4px solid #28a745; padding: 15px; border-radius: 6px; margin-bottom: 30px;">
                    <p style="color: #1b5e20; font-size: 13px; line-height: 1.6; margin: 0;">
                        <strong>✓ Éxito:</strong> Tu solicitud #{{solicitudId}} ha sido completada correctamente. Todos los documentos están firmados y listos.
                    </p>
                </div>

                <!-- CTA Principal -->
                <div style="text-align: center; margin-bottom: 30px;">
                    <a href="{{urlSolicitudes}}" style="display: inline-block; background: linear-gradient(135deg, #28a745 0%, #207c3b 100%); color: #ffffff; text-decoration: none; padding: 16px 40px; border-radius: 8px; font-weight: 600; font-size: 16px; transition: all 0.3s ease; box-shadow: 0 4px 12px rgba(40, 167, 69, 0.3); border: none; cursor: pointer;">
...
const sendMail = (data) => {
    return new Promise((resolve, reject) => {
        ...
        if (data.type === 'requester') {
            ...
            htmlTemplate = htmlContentForCompletionRequester;
            subject = 'DocSigned: ¡Todas las Firmas Completadas!';
            textPlain = 'Tu solicitud de firma ha sido completada exitosamente. Todos los firmantes han completado el proceso.';
        }
        ...
        // Log inicial
        console.log(`\n🔄 PROCESANDO REEMPLAZO DE VARIABLES (tipo: ${data.type})`);
        console.log(`Variables recibidas:`, JSON.stringify(data.variables));
        console.log(`Plantilla contiene {{solicitudId}}: ${htmlContent.includes('{{solicitudId}}')}`);
        
        if (data.variables && typeof data.variables === 'object') {
            Object.keys(data.variables).forEach(key => {
                const placeholder = `{{${key}}}`;
                const value = String(data.variables[key]);
                const countBefore = (htmlContent.match(new RegExp(placeholder, 'g')) || []).length;
                htmlContent = htmlContent.replace(new RegExp(placeholder, 'g'), value);
                const countAfter = (htmlContent.match(new RegExp(placeholder, 'g')) || []).length;
                console.log(`   ${placeholder}: encontrados ${countBefore}, reemplazados ${countBefore - countAfter}, valor: "${value}"`);
            });
        }
        console.log(`Plantilla después del reemplazo contiene {{solicitudId}}: ${htmlContent.includes('{{solicitudId}}')}\n`);
